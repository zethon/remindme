...
namespace dotICQ
{
	...
	public class ICQ
	{
		...
		public ICQ(string strUIN, string strPW)
		{
			m_uin = strUIN;		
			m_pw = strPW;
		}

		public void Connect(string strUIN, string strPW)
		{
			m_uin = strUIN;		
			m_pw = strPW;
			Connect();
		...
		public void Connect()
		{
			...
			if (!m_bAuthenticated)
			{
				ip = Dns.Resolve("login.icq.com").AddressList[0];
				port = 5190;
			}
			else
			{
				...
				if (tlv.type == 0x02)
				{
					...
					while (iOffset < (iTlvStart + 4 + tlv.length))
					{
						...
						if (subtlv.type == 0x101)
						{
							...
							OnIMIn(strSender,strMessage,bAway);
							return;
						}
